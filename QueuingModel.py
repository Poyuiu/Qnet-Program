...
class ServiceCounter(Entity):
    ...
    def begin_service(self):
        ...
        if len(self.queue) >= 1:
            ...
            self.env.logger.debug(
                f"Customer {customer.service_no} served at {self.env.now}"
            )
            ...
            self.scheduler.schedule_after(
                30, EventHandler(self, "end_service", [customer])
            )
    ...
    def end_service(self, customer):
        ...
        self.status = 0
        self.env.logger.debug(
            f"Customer {customer.service_no} leaves at {self.env.now}"
        )
...
if __name__ == "__main__":
    ...
    print(
        ...
        sum(counter.waiting_time) / len(counter.waiting_time),
    )
